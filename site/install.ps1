...
# $env:PINCH_INSTALL_DIR when set) and adds that folder to the user's PATH.
...
& {
  ...
  $arch = switch ($cpu) {
    'AMD64' { 'x86_64' }
    'ARM64' { 'aarch64' }
    default { throw "Pinch Points has no build for a $cpu processor" }
  }
  ...
  $archive = "pinch-points-$tag-$target.zip"
  $url = "https://github.com/$repo/releases/download/$tag/$archive"
  $dir = if ($env:PINCH_INSTALL_DIR) { $env:PINCH_INSTALL_DIR } else { Join-Path $env:LOCALAPPDATA 'Programs\pinch-points' }

  $tmp = Join-Path ([IO.Path]::GetTempPath()) ([IO.Path]::GetRandomFileName())
  New-Item -ItemType Directory -Path $tmp | Out-Null
  try {
    Write-Host "Downloading Pinch Points $tag for $target"
    Invoke-WebRequest -UseBasicParsing -Uri $url -OutFile (Join-Path $tmp $archive)
    Expand-Archive -Path (Join-Path $tmp $archive) -DestinationPath $tmp -Force
    New-Item -ItemType Directory -Path $dir -Force | Out-Null
    Copy-Item (Join-Path $tmp 'pinch-points.exe') (Join-Path $dir 'pinch-points.exe') -Force
  } finally {
    Remove-Item $tmp -Recurse -Force -ErrorAction SilentlyContinue
  }

  Write-Host "Installed $dir\pinch-points.exe"
  ...
  Write-Host 'Run it with: pinch-points'
}
