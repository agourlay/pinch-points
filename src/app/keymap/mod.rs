...
//! all a `KeyboardInput` event offers and winit keeps the keymap it parses
//! private. But the same tables are a few calls away, and asking for them
//! directly gives the whole board at start-up, before a key is touched and
//! including the punctuation no player ever presses.
//!
//! The asking lives in [`os`], a module of its own because two of the three
//! platforms answer only through FFI and the rest of the game has no unsafe
//! in it. It answers in W3C `code` names ("KeyW", "Semicolon"), the
//! vocabulary Bevy's [`KeyCode`] is spelled in, so [`binds::key_from_name`]
//! is the whole of the translation.
//!
//! What it cannot answer (a platform with no path, an X server that will
//! not talk to us, a cap that is a dead key or is not ASCII) falls through
//! to the presses and the language: a better first answer, not a
//! replacement.
//!
